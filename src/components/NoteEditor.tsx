    ...
    return [
      new Plugin({
        ...
        appendTransaction(transactions, _oldState, newState) {
          ...
          if (transactions.some((tr) => tr.getMeta(sortTaskListKey))) return null;
...
const BLOCK_STYLES: { value: TextStyle; label: string; prefix?: string; dividerBefore?: boolean }[] = [
          ...
          newState.doc.descendants((node, pos) => {
            ...
            for (let d = $pos.depth; d > 0; d--) {
              if ($pos.node(d).type.name === "blockquote") {
                nested.push({ from: pos, to: pos + node.nodeSize, content: node.content });
                return false;
              }
            }
          });
...
function defaultContentForStartWith(startWith: string) {
  switch (startWith) {
    case "heading":
      return { type: "doc", content: [{ type: "heading", attrs: { level: 2 } }] };
    case "subheading":
      return { type: "doc", content: [{ type: "heading", attrs: { level: 3 } }] };
    case "body":
...
export function NoteEditor({ note, onSave, onDelete, onRestore, onContentChange, newNoteStartWith = "title", autoSortChecked = true, autoFocus = true, supabaseClient, userId, creatorName, readOnly = false }: Props) {
  ...
  const resolveFile = useRef<(file: File) => Promise<{ src: string; fileName: string; fileSize: number } | null>>();
  ...
  const editor = useEditor({
    ...
    onUpdate({ editor }) {
      ...
      saveTimer.current = setTimeout(() => {
        ...
        const removedImages = [...knownImageUrls.current].filter((u) => !currentImages.has(u));
        const removedFiles = [...knownFileUrls.current].filter((u) => !currentFiles.has(u));
    ...
    editorProps: {
      handlePaste(view, event) {
        ...
        const ed = view.dom.closest(".editor")
          ? editor
          : null;
        if (ed) {
      ...
      case "blockquote": {
        ...
        state.doc.nodesBetween(from, to, (node, pos) => {
          if (node.isTextblock) {
            ...
            let inBq = false;
            for (let d = $pos.depth; d > 0; d--) {
              if ($pos.node(d).type.name === "blockquote") { inBq = true; break; }
            }
        ...
        if (count > 0) {
          if (allInBq) {
            ...
            while (editor.can().lift("blockquote")) editor.commands.lift("blockquote");
...
function TableFloatingToolbar({
  ...
  useEffect(() => {
    ...
    function update() {
      ...
      if (!tableNode) { setPos(null); return; }
      ...
      setPos({
        top: tableRect.bottom - scrollRect.top + scrollRef.current.scrollTop + 4,
    ...
    >
      <div className="table-toolbar-group">
        <button
          ...
          title="Add row above"
        >
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M8 3v10M3 8h10" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/><path d="M2 13h12" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" opacity="0.4"/></svg>
        ...
        <button
          ...
          title="Add row below"
        >
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M8 3v10M3 8h10" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/><path d="M2 3h12" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" opacity="0.4"/></svg>
      ...
      <div className="table-toolbar-group">
        <button
          className="table-toolbar-btn"
          onMouseDown={act(() => editor.chain().focus().addColumnBefore().run())}
          title="Add column left"
        >
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M8 3v10M3 8h10" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/><path d="M13 2v12" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" opacity="0.4"/></svg>
        ...
        >
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M8 3v10M3 8h10" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/><path d="M3 2v12" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" opacity="0.4"/></svg>
      ...
      <div className="table-toolbar-group">
        <button
          className="table-toolbar-btn"
          onMouseDown={act(() => editor.chain().focus().toggleHeaderRow().run())}
          title="Toggle header row"
        >
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><rect x="2" y="3" width="12" height="10" rx="1.5" stroke="currentColor" strokeWidth="1.3"/><path d="M2 6.5h12" stroke="currentColor" strokeWidth="1.3"/></svg>
        ...
        <button
          ...
          title="Merge cells"
        >
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><rect x="2" y="3" width="12" height="10" rx="1.5" stroke="currentColor" strokeWidth="1.3"/><path d="M5.5 8h5M8.5 5.5L10.5 8l-2 2.5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/></svg>
        ...
        <button
          ...
          title="Split cell"
        >
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><rect x="2" y="3" width="12" height="10" rx="1.5" stroke="currentColor" strokeWidth="1.3"/><path d="M10.5 8h-5M5.5 5.5L3.5 8l2 2.5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/></svg>
